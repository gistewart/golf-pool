      ...
      if (!document[hidden]) {
        ...
        async function onTheRange() {
          ...
          for (let i = 0; i < fieldData.length; i++) {
            ...
            for (let j = 0; j < fieldData[i].Players.length; j++) {
              ...
              fieldData[i].Players[j].Results = fieldData[i].Players[
                j
              ].Results.slice(0, 5);
        ...
        async function liveEvent() {
          ...
          for (let i = 0; i < ams.length; i++) {
            for (let j = 0; j < livePositions.length; j++) {
              if (ams[i] == livePositions[j].playerName) {
                livePositions[j].playerName =
                  livePositions[j].playerName + " (a)";
            ...
            for (let i in hCapTbl) {
              for (let j = 0; j < livePositions.length; j++) {
                if (hCapTbl[i].playerName === livePositions[j].playerName) {
                  hCapTbl[i].gross =
                    livePositions[j].toParAdj - hCapTbl[i].handicap;
                  break;
                }
              }
            }
          ...
          if (tenShotRule) {
            ...
            let leaderScore = livePositions[0].toPar
              .replace("E", 0)
              .replace("+", "");
            ...
            if (purseArr[i].pos > 0) {
              if (purseArr[i].data[0].count === 1) {
                purseArr[i].data[0].avgPercent =
                  typeof livePurseSplit[purseArr[i].pos - 1 - amTotal] ===
                  "undefined"
                    ? 0
                    : purseArr[i].pos < 3
                    ? Number(
                        livePurseSplit[purseArr[i].pos - 1 - amTotal].percent
                      )
                    : Number(
                        livePurseSplit[purseArr[i].pos - 1 - amTotal].percent
                      ) * usOpenPurseAdjFactor;
                purseArr[i].data[0].dollars =
                  (purseArr[i].data[0].avgPercent * liveSchedule[0].purse) /
                  100;
                ...
                for (
                  let j = 0;
                  j < purseArr[i].data[0].count - posAmCount;
                  j++
                ) {
                  purseSum +=
          ...
          for (let i = 0; i < livePlayers.length; i++) {
            for (let j = 0; j < partResult.length; j++) {
              if (livePlayers[i].id === partResult[j].id) {
                ...
                livePlayers[i]["livePriorEarnings"] =
                  partResult[j].poolsterEarnings;
                break;
              }
            }
          }
          ...
          for (let i = 0; i < livePlayers.length; i++) {
            ...
            for (let j = 0; j < livePlayers[i].Players.length; j++) {
              ...
              for (
                let k = 0;
                k < livePlayers[i].Players[j].Tournaments.length;
                k++
              ) {
          ...
          for (let i in livePlayers) {
            livePlayers[i].liveNewEarnings =
              livePlayers[i].livePriorEarnings +
              livePlayers[i].poolsterEarnings;
          }
              ...
              for (let p = 0; p < sortedPartResult.length; p++) {
                if (sorted[f].poolster == sortedPartResult[p].poolster) {
                  ...
                  sorted[f].ranking < sorted[f].priorRanking
                    ...
                    : "nc";
                  sorted[f].rankingChangeAbs = Math.abs(
                    sorted[f].rankingChange
                  );
                }
              }
          ...
          for (let i = 0; i < sorted.length; i++) {
            ...
            for (let j = 0; j < sorted[i].Players.length; j++) {
              ...
              if (apiCall === "Season") {
                for (
                  let k = 0;
                  k < sorted[i].Players[j].tournaments.length;
                  k++
                ) {
                  $(".leaderboard-container").append(
                    "<tr class='level3 hiddenRow collapse' id='demo-" +
                      ...
                      "</td><td class='earnings'>" +
                      sorted[i].Players[j].tournaments[
                        k
                      ].earnings.toLocaleString("us-US", {
                        style: "currency",
                        currency: "USD",
                        minimumFractionDigits: 0,
                        maximumFractionDigits: 0,
